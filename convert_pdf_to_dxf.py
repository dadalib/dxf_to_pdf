...
"""
Advanced PDF to DXF Converter (PyMuPDF + ezdxf)
================================================
Handles lines, rectangles, circles, polygons, Bezier curves, and paths.
Approximates curves as polylines for better DXF compatibility.
"""
...
def pdf_to_dxf(pdf_path, dxf_path, scale=1.0, curve_segments=20):
    """
    ...
    Parameters:
        ...
        curve_segments (int): Number of segments to approximate curves.
    """
    # Normalise paths
        ...
        for d in drawings:
            for item in d["items"]:
                ...
                if t == "l":  # line
                    ...
                    _, points, *_ = item
                    scaled_points = [(x*scale, y*scale) for x, y in points]
                    # Approximate Bezier curve as polyline
                    msp.add_lwpolyline(scaled_points)

                elif t == "path":  # complex path
                    _, path_points, *_ = item
